...
"$NODE" -e '
  ...
  if (value.currentVersion !== "v1.5.15" || value.latestVersion !== "v9.8.7") process.exit(1);
...
/bin/cp "$ROOT/assets/portal-hero.png" "$ROTATE_STATE/images/b.png"
...
/usr/bin/env HOME="$ROTATE_HOME" NODE="$NODE" \
  "$ROOT/scripts/rotate-images-macos.sh" tick
[ "$(/usr/bin/head -n1 "$ROTATE_STATE/rotation/current-image")" = 'b.png' ]
/bin/rm -f "$ROTATE_STATE/images/a.png" "$ROTATE_STATE/images/b.png"
...
/usr/bin/env -u HOME /bin/bash -c '. "$1/scripts/common-macos.sh"; [ -n "$HOME" ] && [ "$SKIN_VERSION" = "1.5.15" ]' _ "$ROOT"
