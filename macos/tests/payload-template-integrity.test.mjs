...
// The payload installs a callable renderer function and immediately invokes it.
// Injecting a `return` as the first statement recovers exactly the arguments the
// renderer would have received without running any renderer logic.
function readPayloadArguments(payload) {
  const marker = ") => {";
  const at = payload.indexOf(marker);
  assert.notEqual(at, -1, "payload must keep the canonical renderer installer signature");
  const probe = `${payload.slice(0, at + marker.length)}
return { cssText, artDataUrl, themeConfig };
${payload.slice(at + marker.length)}`;
  return vm.runInNewContext(probe, { window: {} }, { timeout: 10_000 });
}
