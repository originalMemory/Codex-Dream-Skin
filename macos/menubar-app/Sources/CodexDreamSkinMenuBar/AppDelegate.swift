...
final class AppDelegate: NSObject, NSApplicationDelegate, NSMenuDelegate, UNUserNotificationCenterDelegate {
    ...
    private var rotationStatusRefreshRunning = false
    private var rotationTickRunning = false
    private var rotationEnabled = false
    private var rotationInterval = 60
    private var rotationCurrent = ""
    private var rotationError = ""
    ...
    private let requiredEngineRelativePaths = [
        ...
        "scripts/rotate-images-macos.sh",
    ...
    func application(_ application: NSApplication, open urls: [URL]) {
        guard urls.count == 1,
              let versionID = CommunityThemeContract.versionID(from: urls[0])
        else {
            showError(
                title: "一键换肤链接无效",
    ...
    private func addThemeMenu(enabled: Bool) {
        ...
        addRotationMenu(enabled: enabled, to: submenu)
    ...
    private func addRotationMenu(enabled: Bool, to destination: NSMenu) {
        let root = NSMenuItem(title: "自动换图", action: nil, keyEquivalent: "")
        let submenu = NSMenu(title: "自动换图")
        submenu.autoenablesItems = false
        let status = rotationEnabled
            ? "运行中 · \(rotationInterval) 秒"
            : "已停止 · \(rotationInterval) 秒"
        addDisabledItem(status, to: submenu)
        if !rotationCurrent.isEmpty {
            addDisabledItem("当前：\(cleanMenuText(rotationCurrent))", to: submenu)
        }
        if !rotationError.isEmpty {
            addDisabledItem("错误：\(cleanMenuText(rotationError))", to: submenu)
        }
        addActionItem(
            rotationEnabled ? "停止" : "启动",
            action: rotationEnabled ? #selector(stopRotation) : #selector(startRotation),
            enabled: enabled,
            to: submenu
        )
        let intervalRoot = NSMenuItem(title: "间隔", action: nil, keyEquivalent: "")
        let intervalMenu = NSMenu(title: "间隔")
        intervalMenu.autoenablesItems = false
        for seconds in [60, 300, 900, 1800] {
            let minutes = seconds / 60
            let title = "\(minutes) 分钟" + (rotationInterval == seconds ? " ✓" : "")
            let item = addActionItem(
                title,
                action: #selector(setRotationInterval(_:)),
                enabled: enabled,
                to: intervalMenu
            )
        ...
        addActionItem(
            "自定义秒数…",
            action: #selector(promptRotationInterval),
            enabled: enabled,
            to: intervalMenu
        )
        intervalRoot.submenu = intervalMenu
        submenu.addItem(intervalRoot)
        root.submenu = submenu
    ...
    private func refreshStatus() {
        guard !statusRefreshRunning,
              let script = installedScript(named: "status-dream-skin-macos.sh")
        else {
            return
        }
    ...
    private func refreshRotationStatus() {
        guard !rotationStatusRefreshRunning,
              let script = installedScript(named: "rotate-images-macos.sh")
        else {
            return
        }
        rotationStatusRefreshRunning = true
        ScriptRunner.run(script: script, arguments: ["status"]) { [weak self] result in
            guard let self else { return }
            self.rotationStatusRefreshRunning = false
            guard result.succeeded else { return }
            let values = [String: String](
                uniqueKeysWithValues: result.output.split(separator: "\n").compactMap {
            ...
            self.rotationEnabled = values["enabled"] == "true"
            self.rotationInterval = Int(values["interval"] ?? "") ?? 60
            self.rotationCurrent = values["current"] ?? ""
            self.rotationError = values["error"] ?? ""
            self.rebuildMenu()
        }
    }

    private func tickRotation() {
        guard !rotationTickRunning,
              let script = installedScript(named: "rotate-images-macos.sh")
        else {
            return
        }
        rotationTickRunning = true
        ScriptRunner.run(script: script, arguments: ["tick"]) { [weak self] _ in
            guard let self else { return }
            self.rotationTickRunning = false
            self.refreshRotationStatus()
        }
    }

    @objc private func startRotation() {
        runInstalledScript(
            named: "rotate-images-macos.sh",
            arguments: ["start"],
            operation: "启动自动换图"
        )
    }
    ...
    @objc private func setRotationInterval(_ sender: NSMenuItem) {
        ...
        runInstalledScript(
            named: "rotate-images-macos.sh",
            arguments: ["set-interval", String(seconds)],
            operation: "设置自动换图间隔"
        )
    }

    @objc private func promptRotationInterval() {
        let alert = NSAlert()
        alert.messageText = "设置自动换图间隔"
        alert.informativeText = "请输入秒数，最小 10 秒。"
        let field = NSTextField(string: String(rotationInterval))
        ...
        guard alert.runModal() == .alertFirstButtonReturn,
              let seconds = Int(field.stringValue),
              seconds >= 10
        else {
            return
        }
    ...
    private func beginCommunityThemeApply(versionID: String) {
        ...
        ScriptRunner.run(script: statusScript, arguments: ["--json", "--deep"]) { [weak self] result in
            ...
            guard result.succeeded,
                  ...
                  current.isReadyForCommunityApply
            else {
        ...
        communityHTTP.get(
            ...
            do {
                guard case let .success(payload) = result,
                      ...
                      payload.body.count == Int(metadata.packageBytes)
                else {
    ...
    private func captureActiveThemeThenApplyImported(
        ...
        guard CommunityThemeContract.isVersionID(metadata?.id ?? ""),
              ...
              let transactionScript = installedScript(named: "apply-community-theme-macos.sh")
        else {
            finishThemeOperation(cleanupRoot: cleanupRoot)
            showError(
                title: "主题已导入，但没有应用",
    ...
    @objc private func openCodex() {
        ...
        guard !engineNeedsInstall(),
              let script = installedScript(named: "start-dream-skin-macos.sh")
        else {
    ...
    @objc private func checkForUpdates() {
        ...
        guard let script = installedScript(named: "check-update-macos.sh")
            ?? bundledScript(named: "check-update-macos.sh")
        else {
        ...
        ScriptRunner.run(script: script, arguments: ["--json"]) { [weak self] result in
            ...
            guard result.succeeded,
                  ...
                  let available = value["updateAvailable"] as? Bool
            else {
                self.showError(
                    title: "检查更新失败",
    ...
    @objc private func openAvailableUpdate() {
        guard let url = URL(
            string: availableUpdate?.releaseURL
                ?? "https://github.com/Fei-Away/Codex-Dream-Skin/releases/latest"
        ) else { return }
        NSWorkspace.shared.open(url)
    }
