...
- 所有图片比例都可把背景延伸到侧边栏；原生菜单栏 App 与旧 SwiftBar 均可从图片目录按文件名定时轮换，默认 60 秒并允许配置。
...
- 左侧栏主动收起时，只要主壳、主区域和输入框仍可见，主题应用与自动换图不再误报“显示校验未通过”；缺少其他 L1 壳层锚点的页面仍保持拒绝。
- 图片轮换只把新图片和主题作为 CDP 参数传入现有 renderer，不再为每次换图重建并执行整份注入源码；旧 Blob URL 仍在替换时释放。
- 自动换图成功时不再显示「正在应用 / 已应用」提示；只有图片处理、注入或显示校验失败时才显示错误。
- 不再向隐藏的头像浮层 renderer 注入和轮换整张背景图，并会释放后来不再符合主界面特征的注入会话。
- Codex 窗口不可见时自动换图会在写入主题前延后；显示校验失败也会终止本轮，不再每十秒遍历整组图片。
- 修复原生菜单栏 App 的精简环境导致自动换图在可见性检查前因 `NODE` 未定义而静默退出；同时补入 v1.5.12 安装包遗漏的图片尺寸预检脚本。
