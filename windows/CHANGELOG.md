...
- 系统托盘支持按文件名顺序自动轮换本地图片目录，默认 60 秒，可配置间隔；暂停或完全恢复时自动停止。

### 改进

- 横图、方图和竖图都使用单张整窗背景并延伸到侧边栏；宽高比只保留用于焦点、裁切和任务页构图。

### 修复

- 自动换图改为调用已注入的稳定 Renderer 安装函数，图片通过 CDP 参数传递，不再每分钟重复注册和编译数 MB 的 Base64 脚本，避免长时间轮换后 Renderer 内存持续增长。
