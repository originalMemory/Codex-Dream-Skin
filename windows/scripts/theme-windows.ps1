...
function Get-DreamSkinRotationState {
  ...
  if ($null -ne $state -and $null -ne $state.intervalSeconds) {
    $parsed = 0
    if ([int]::TryParse("$($state.intervalSeconds)", [ref]$parsed) -and $parsed -ge 10) {
      $interval = $parsed
    }
  }
  return [pscustomobject]@{
    ...
    currentImage = if ($null -ne $state -and $state.currentImage) {
      "$($state.currentImage)"
    } else { '' }
    lastChangeUtc = if ($null -ne $state -and $state.lastChangeUtc) {
      "$($state.lastChangeUtc)"
    } else { '' }
    lastError = if ($null -ne $state -and $state.lastError) {
      "$($state.lastError)"
    } else { '' }
  }
}
...
function Invoke-DreamSkinRotationTick {
  param([string]$StateRoot = (Join-Path $env:LOCALAPPDATA 'CodexDreamSkin'))
  $state = Get-DreamSkinRotationState -StateRoot $StateRoot
  if (-not $state.enabled -or (Test-DreamSkinPaused -StateRoot $StateRoot)) {
    return $false
  }
  ...
  for ($itemIndex = 0; $itemIndex -lt $images.Count; $itemIndex += 1) {
    if ($images[$itemIndex].Name.Equals(
      $state.currentImage,
  ...
  for ($offset = 0; $offset -lt $images.Count; $offset += 1) {
    ...
    $null = Set-DreamSkinActiveTheme -ImagePath $next.FullName -Theme $null `
      -Name $next.BaseName -ArchiveImage $false -StateRoot $StateRoot
